...
class joint_knowledge_rank_model:
    def __init__(self, intent_num_classes, learning_rate, decay_steps, decay_rate, sequence_length,
        ...
        self.x = tf.placeholder(tf.int32, [None, self.sequence_length], name="x") #input
        self.x_context = tf.placeholder(tf.int32, [None, self.sequence_length], name="x_context") #context
        ...
        self.y_intent_previous = tf.placeholder(tf.int32, [None],name="y_intent_previous")
        ...
        self.logits_intent,intent_hidden_states = self.inference_intent(self.inputs_concat) #[none,intent_num_classes]
        self.logits_slots = self.inference_slot(intent_hidden_states) #[none,sequence_length,slots_num_classes]

        self.logits_intent_previous,intent_hidden_states_previous = self.inference_intent(self.x_context)  #[none,intent_num_classes]
        self.logits_slots_inherit_intent = self.inference_slot(intent_hidden_states_previous)  #[none,sequence_length,slots_num_classes]
    ...
    def inference_domain(self): #domain
        with tf.variable_scope("hidden_layer_domain"): #some inference structure with intent, but parameters is not the same.
            hidden_states=self.conv_layer(self.inputs_concat) #[None,num_filters_total]
    ...
    def inference_intent(self,inputs_concat): #intent
        with tf.variable_scope("hidden_layer_intent"):
            hidden_states=self.conv_layer(inputs_concat) #[None,num_filters_total]
        ...
        return logits,hidden_states_concat

    def inference_slot(self,intent_hidden_states): #slot
        ...
        for i in range(self.sequence_length):
            ...
            feature=tf.concat([feature,intent_hidden_states],axis=1) #ADD.201712.20.slot contional on intent#[none,self.hidden_size*2+self.num_filters_total*2]
    ...
    def conv_layer(self,inputs):
        ...
        self.inputs_representation_expanded = tf.expand_dims(inputs,-1)  # [None,sentence_length,hidden_size*2,1]. expand dimension so meet input requirement of 2d-conv
    ...
    def loss_seq2seq(self):
        with tf.name_scope("loss"):
            ...
            loss_intent_previous= tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.y_intent_previous, logits=(self.logits_intent_previous+self.epsilon())) #[batch_size].#A `Tensor` of the same shape as `labels`
            loss_intent_previous=tf.reduce_mean(loss_intent_previous) #scalar
            ...
            weights_intent_previous=tf.nn.sigmoid(tf.cast(self.global_step/1000,dtype=tf.float32))/4 #0-0.25
            weights_domain = tf.nn.sigmoid(tf.cast(self.global_step / 1000, dtype=tf.float32)) / 4   #0-0.25
            weights=weights_intent+weights_domain+weights_intent_previous
            ...
            loss = (1.0-weights)*loss_slot+weights_intent*loss_intent+weights_intent_previous*loss_intent_previous+ weights_domain*loss_domain+ l2_losses #loss = (1.0-weights)*loss_slot+weights*loss_intent + l2_losses
            return loss
